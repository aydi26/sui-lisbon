...
| *"So how do I know the Move code works if it's never run on chain?"* | You do not, and we will not pretend otherwise. What you can check is: 275 Move tests, twelve structural gates, and 46 golden fixtures the TypeScript and the Move consume from the same file. The gate we are still owed is exactly the one that needs a chain — the byte-for-byte `devInspect` comparison — and it is the first thing we turn on after the publish lands. |

...
use aphotic::envelope::{Self, EnvelopeParams};
...
fun base_params(): EnvelopeParams {
    envelope::new_envelope_params(50, 100_000_000, 1_000, 100, LIVE_REFILL, LIVE_CAP, 0)
}
...
fun accessors_round_trip_the_constructor() {
    let p = envelope::new_envelope_params(50, 100_000_000, 1_000, 100, LIVE_REFILL, LIVE_CAP, 7);

    assert!(envelope::max_slippage_bps(&p) == 50, 0);
    assert!(envelope::max_notional_per_epoch_sats(&p) == 100_000_000, 1);
    assert!(envelope::min_cooldown_ms(&p) == 1_000, 2);
    assert!(envelope::buffer_ratio_bps(&p) == 100, 3);
    assert!(envelope::limiter_refill_rate(&p) == LIVE_REFILL, 4);
    assert!(envelope::limiter_max_capacity(&p) == LIVE_CAP, 5);
    assert!(envelope::epoch_start_ms(&p) == 7, 6);

    // Epoch accounting starts empty, so the FIRST action is never cooled down.
    assert!(envelope::epoch_notional_used_sats(&p) == 0, 7);
    assert!(envelope::last_action_ms(&p) == 0, 8);

    // The two T4.1 fields are DEFAULTED, keeping the constructor at 7 arguments (delta (b)).
    assert!(envelope::max_divergence_bps(&p) == envelope::default_max_divergence_bps(), 9);
    assert!(envelope::epoch_len_ms(&p) == envelope::default_epoch_len_ms(), 10);
    assert!(envelope::default_max_divergence_bps() == 200, 11);
    assert!(envelope::default_epoch_len_ms() == 86_400_000, 12);
    assert!(envelope::bps_denominator() == 10_000, 13);
}
...
fun setters_are_the_only_way_to_move_a_threshold() {
    ...
    envelope::set_max_divergence_bps(&mut p, 25);
    envelope::set_epoch_len_ms(&mut p, 0);

    assert!(envelope::max_divergence_bps(&p) == 25, 0);
    assert!(envelope::epoch_len_ms(&p) == 0, 1);

    // Nothing else moved.
    assert!(envelope::max_slippage_bps(&p) == 50, 2);
    assert!(envelope::buffer_ratio_bps(&p) == 100, 3);
}
...
fun buffer_is_the_static_floor_when_the_bridge_can_serve() {
    ...
    let buffer = envelope::buffer_sats(100_000_000, 5_000_000, LIVE_CAP, &p);
...
fun buffer_holds_the_unserviceable_remainder() {
    ...
    let buffer = envelope::buffer_sats(100_000_000, 9_000_000, 2_000_000, &p);
    assert!(buffer == 7_000_000, 0);

    // ...and that dominates the 1_000_000 static floor, which is what `max` is for.
    assert!(buffer > envelope::buffer_sats(100_000_000, 0, LIVE_CAP, &p), 1);
}
...
fun static_fallback_governs_at_zero_capacity() {
    ...
    assert!(envelope::buffer_sats(100_000_000, 0, 0, &p) == 1_000_000, 0);

    // A keeper attesting capacity == 0 AND demand == 0 cannot push the buffer below the floor —
    // that is the whole trust bound.
    assert!(envelope::buffer_sats(100_000_000, 0, 0, &p) >= 1_000_000, 1);
}
...
fun deployable_subtracts_the_earmark_before_the_buffer() {
    ...
    let nav = 70_000_000;
    let deployable = envelope::deployable_sats(100_000_000, nav, 30_000_000, 0, LIVE_CAP, &p);
    ...
    let without_earmark = envelope::deployable_sats(100_000_000, nav, 0, 0, LIVE_CAP, &p);
...
fun deployable_never_underflows_and_never_exceeds_idle() {
    ...
    assert!(envelope::deployable_sats(1_000, 100_000_000, 0, 0, LIVE_CAP, &p) == 0, 0);

    // Earmark larger than idle -> 0.
    assert!(envelope::deployable_sats(1_000, 1_000, MAX_U64, 0, LIVE_CAP, &p) == 0, 1);

    // Unserviceable demand at u64::MAX -> 0, no overflow anywhere.
    assert!(envelope::deployable_sats(MAX_U64, MAX_U64, 0, MAX_U64, 0, &p) == 0, 2);

    // deployable <= idle for every combination above, and for a zero-buffer envelope.
    let zero_buffer = envelope::new_envelope_params(50, MAX_U64, 0, 0, LIVE_REFILL, LIVE_CAP, 0);
    assert!(envelope::deployable_sats(12_345, 12_345, 0, 0, LIVE_CAP, &zero_buffer) == 12_345, 3);

    // A 100 % buffer ratio strands everything.
    let all_buffer = envelope::new_envelope_params(50, MAX_U64, 0, 10_000, LIVE_REFILL, LIVE_CAP, 0);
    assert!(envelope::deployable_sats(12_345, 12_345, 0, 0, LIVE_CAP, &all_buffer) == 0, 4);
}
...
fun divergence_and_slippage_bps_are_exact() {
    ...
    assert!(envelope::divergence_bps(101_000_000_000_000, 100_000_000_000_000) == 100, 0);
    // ...and symmetric below it.
    assert!(envelope::divergence_bps(99_000_000_000_000, 100_000_000_000_000) == 100, 1);
    assert!(envelope::divergence_bps(BOOK_MID, BOOK_MID) == 0, 2);

    // Slippage is measured against the BOOK mid (G9: never a raw oracle price).
    assert!(envelope::slippage_bps(BOOK_MID + BOOK_MID / 100, BOOK_MID) == 100, 3);
    assert!(envelope::slippage_bps(BOOK_MID - BOOK_MID / 200, BOOK_MID) == 50, 4);
    assert!(envelope::slippage_bps(BOOK_MID, BOOK_MID) == 0, 5);
}
...
fun divergence_fails_closed_on_a_missing_price() {
    ...
    assert!(envelope::divergence_bps(BOOK_MID, 0) == MAX_U64, 0);
    assert!(envelope::divergence_bps(0, BOOK_MID) == MAX_U64, 1);
    assert!(envelope::divergence_bps(0, 0) == MAX_U64, 2);
    assert!(envelope::slippage_bps(BOOK_MID, 0) == MAX_U64, 3);
}
...
fun strategy_availability_gate() {
    envelope::assert_strategy_available(&b"blob-v0", true);
}

#[test]
#[expected_failure(abort_code = envelope::EBlobUnavailable)]
fun strategy_unavailable_aborts() {
    envelope::assert_strategy_available(&b"blob-v0", false);
}

#[test]
#[expected_failure(abort_code = envelope::EBlobUnavailable)]
fun strategy_with_an_empty_blob_id_aborts() {
    envelope::assert_strategy_available(&vector[], true);
}
...
fun check_action_succeeds_and_advances_exactly_once() {
    ...
    assert!(envelope::epoch_notional_used_sats(&p) == 1_000_000, 0);
    assert!(envelope::last_action_ms(&p) == 5_000, 1);
    ...
    assert!(envelope::epoch_notional_used_sats(&p) == 3_000_000, 2);
    assert!(envelope::last_action_ms(&p) == 9_000, 3);
...
fun check_action_emits_the_receipt() {
    ...
    let events = event::events_by_type<envelope::EnvelopeChecked>();
    assert!(events.length() == 1, 0);
    let (vault_id, notional, deployable, capacity, mid) =
        envelope::envelope_checked_fields(events.borrow(0));
...
#[test]
#[expected_failure(abort_code = envelope::EPaused)]
...
#[test]
#[expected_failure(abort_code = envelope::ECooldown)]
fun check_action_aborts_on_cooldown() {
    ...
    check(&mut p, 100_000_000, 0, 0, LIVE_CAP, 1_000_000, BOOK_MID, BOOK_MID, &clk);
    assert!(envelope::last_action_ms(&p) == 5_000, 0);
...
fun first_action_is_never_cooled_down() {
    ...
    assert!(envelope::last_action_ms(&p) == 0, 1);
...
#[test]
#[expected_failure(abort_code = envelope::EOracleDivergence)]
...
#[test]
#[expected_failure(abort_code = envelope::ENotionalCap)]
fun check_action_aborts_on_the_epoch_notional_cap() {
    ...
    assert!(envelope::epoch_notional_used_sats(&p) == 60_000_000, 0);
...
#[test]
#[expected_failure(abort_code = envelope::ESlippage)]
...
fun the_epoch_notional_budget_rolls() {
    ...
    assert!(envelope::epoch_notional_used_sats(&p) == 90_000_000, 0);
    assert!(envelope::epoch_start_ms(&p) == 0, 1);

    // One full day later the window rolls: the budget is fresh, so 90m fits again.
    clk.set_for_testing(86_400_000);
    check(&mut p, 200_000_000, 0, 0, LIVE_CAP, 90_000_000, BOOK_MID, BOOK_MID, &clk);
    assert!(envelope::epoch_notional_used_sats(&p) == 90_000_000, 2);
    assert!(envelope::epoch_start_ms(&p) == 86_400_000, 3);
    ...
    assert!(envelope::epoch_start_ms(&p) == 86_400_000 * 5, 4);
    assert!(envelope::epoch_notional_used_sats(&p) == 1_000, 5);
...
fun a_zero_epoch_len_never_rolls() {
    ...
    let mut clk = clock_at(&mut scenario, 1_000);
    let mut p = base_params();
    envelope::set_epoch_len_ms(&mut p, 0); // one open-ended epoch
